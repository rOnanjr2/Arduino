...
CS1694.displaySetup(true); //set chip up as 7x10 mode
  CS1694.setBrightness(1); //set full brightness
...
if(bitRead(inputButtons,18)){ //when the button is pressed bit = 1
  delay(60);
   digitVal ++;
  ...
  if(bitRead(inputButtons,19)){
    delay(60);
 digitVal --;
  if(digitVal > 37){digitVal = 37;}
  CS1694.dispNum(digitVal,editDigit); 
  }
if(bitRead(inputButtons,20)){
  delay(250);
  editDigit --;
